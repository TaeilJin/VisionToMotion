...
public class HumanEnvDataExtraction_IMG : RealTimeAnimation
{
    ...
    public CameraImageMap CamMap;
    public string image_data_path;
    ...
    public void SetupSensors()
    {
        ...
        CamMap = new CameraImageMap();
        // 생성
        CamMap.SetCamera(actor_source.Bones[0].Transform.position,
        verticalCount: 4, horizontalCount: 8, radius: 3f,
        image_data_path);
        ...
        if (b_play)
        {
            ...
            if (Frame == StartFrame)
            {
                ...
                if (b_write)
                {
                    ...
                    Debug.Log("Extraction Folder Name : " + output_folder);
                    ...
                    // ★ 확장자 2중 부착 방지
                    string baseName = Path.GetFileNameWithoutExtension(_MotionData.FileName);
                    // string filePath = Path.Combine(output_folder, baseName + ".txt");
                    // Debug.Log("Record file: " + filePath);
                    File_record = CreateFile(output_folder, baseName, false, ".txt");
                }
            }
            // �������� ������ ������ ������ ������ ���´�.
            ...
            else
            {
                ...
                // 위치 갱신
                Matrix4x4 centerMat = Matrix4x4.TRS(actor_source.Bones[0].Transform.position, actor_source.Bones[0].Transform.rotation, Vector3.one);
                CamMap.UpdateCamera(centerMat);


                if (b_write && CamMap.Image_data_path != "None")
                {
                    Vector3 root_vel;
                    Vector3[] Velocity_Forward;
                    Vector3[] Velocity_Up;
                    ...
                    CamMap.SaveImage(CamMap.Image_data_path);
    ...
    [CustomEditor(typeof(HumanEnvDataExtraction_IMG), true)]
    public class HumanEnvDataExtraction_IMG_Editor : Editor
    {
        public HumanEnvDataExtraction_IMG Target;
        ...
        public void Awake()
        {
            Target = (HumanEnvDataExtraction_IMG)target;
        ...
        private void Inspector()
        {
            ...
            if (Target.CamMap != null)
            {
                Target.CamMap.inspector();

                EditorGUILayout.BeginHorizontal();
                if (Utility.GUIButton("Set Image Data Folder", Color.white, Color.red))
                {
                    EditorApplication.delayCall += () =>
                    {
                        string dataPath = EditorUtility.OpenFolderPanel("Text File Folder", "", "Assets");
                        Target.image_data_path = dataPath;
                    };
                }
                EditorGUILayout.EndHorizontal();
                
                EditorGUILayout.LabelField("ImageData_Folder", GUILayout.Width(200f));
                Target.image_data_path = EditorGUILayout.TextField(Target.image_data_path, GUILayout.Width(200f));
